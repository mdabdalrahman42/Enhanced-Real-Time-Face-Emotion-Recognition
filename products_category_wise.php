...
$emotion = urldecode($_GET['emotion']);
                ...
                <div class="container-fluid">
                    ...
                    <div class="collapse navbar-collapse" id="navbarSupportedContent">
                        <ul class="navbar-nav mb-2 mb-lg-0 start">
                            ...
                            <a class="nav-link text-primary" aria-current="page" href="' . $_SERVER['PHP_SELF'] . '?id=' . urlencode($id) . '&emotion=' . urlencode($emotion) . '">Products</a>

                            </li>
                            <li class="nav-item second">
                                <a class="nav-link active" aria-current="page" href="cart.php?id='. urlencode($id) .'&emotion='. urlencode($emotion) .'">Container<svg
                            ...
                            <li class="nav-item third">
                                <a class="nav-link active" aria-current="page" href="profile.php?id='. urlencode($id) .'&emotion='. urlencode($emotion) .'">Profile<svg
                            ...
                            <li class="nav-item current">
                                <a class="nav-link text-primary" aria-current="page" href="emotion.php?id='. urlencode($id) .'&emotion='. urlencode($emotion) .'">'. $emotion . $emoji .'</a>
                            </li>
                            <li class="nav-item">
...
$ages = ['Babies', 'Children', 'Adolescents', 'Young Adults', 'Middle Adults', 'Elder Adults', 'All'];

$ranges = ['[0 - 5 Years]', '[6 - 12 Years]', '[13 - 18 Years]', '[19 - 35 Years]', '[36 - 60 Years]', '[60+ Years]', ''];
        
$Dbtns = "dropdown-btn-";
        
$Dcontents = "dropdown-content-";
        
$Dtoggles = "dropdown-toggle-";
        
$productQuery = "SELECT * from product_data WHERE Emotion = '$emotion' ORDER BY RAND()";
...
if ($productResult->num_rows > 0) 
{
    ...
    for ($i = 0; $i < 3; $i++) {
        ...
        $productGenderQuery = "SELECT * from product_data WHERE Emotion = '$emotion' AND Gender = '$genders[$i]' ORDER BY RAND()";

        $productGenderResult = $conn->query($productGenderQuery);


        if ($productGenderResult->num_rows > 0) {
        ...
        <div class="row rowsmall m-3 mt-4">

            <div class="col">
        
                <div class="dropdown">
                <button class="dropdown-togg '. $Dtoggles . $k .'" id="'. $Dbtns . $k .'">'. $genders[$i] .' <span class="dropdown-toggle"></span></button>
                    <div class="dropdown-content" id="'. $Dcontents . $k .'">';

            for ($j = 0; $j < 7; $j++) {

                $productAgeQuery = "SELECT * FROM product_data WHERE Emotion = '$emotion' AND Gender = '$genders[$i]' AND `Age Group` = '$ages[$j]' ORDER BY RAND()";

                $productAgeResult = $conn->query($productAgeQuery);
                
                if ($productAgeResult->num_rows > 0) {
                    ...
                    echo '
                        <div class="row m-3">
                            <div class="col">
                                <div class="dropdown">
                                <button class="dropdown-togg '. $Dtoggles . $k . $l .'" id="'. $Dbtns . $k . $l .'">'. $ages[$j] .' '. $ranges[$j] .' <span class="dropdown-toggle"></span></button>
                                    <div class="dropdown-content" id="'. $Dcontents . $k . $l .'">
                                        <div class="row">';

                                while ($row = $productAgeResult->fetch_assoc()) {

                                    echo '
                                            <div class="col mt-4 mb-4 card-container d-flex justify-content-center align-items-center">

                                                <div class="card mx-auto">
                                                
                                                    <img src="static/images/Products/'. $row['Product Id'] .'.jpg" height="320px" class="card-img-top border-bottom border-body-tertiary" alt="...">
                                    
                                                    <div class="card-body d-flex flex-column justify-content-center">
                                        
                                                        <h5 class="card-title mb-2 text-center">'. $row['Product Name'] .' <span class="brand">(By '. $row['Brand Name'] .')</span></h5>
                                                        <p class="card-text text-center mt-1 mb-2">'. $row['Color'] .' | '. $row['Material'] .' | '. $row['Net Quantity'] .'</p>
                                                        <p class="card-text text-center cost mb-2">Rs. '. $row['Price'] .'</p>
                                                        <a href="#" class="btn btn-primary add mt-1">Add to Cart</a>
                                                    </div>
...
else
{
    ...
    <h6 class="error-msg">No products found for the emotion ' . $emotion .'!!!</h6>
